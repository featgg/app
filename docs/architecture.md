...
A narrow exception to "a data source is always concrete" exists for testability. When a repository's single source is the client SDK and that SDK's call surface cannot be substituted in a test under this repo's hand-rolled-fake convention — its query chain is built from concrete SDK types that cannot be subclassed or stubbed without a general-purpose mocking library — a thin **abstract** data-source seam may be declared in `data/` solely so the repository implementation can be exercised against a hand-written fake. The seam still obeys the DTO rule: its concrete implementation wraps the SDK call and parses the response into a typed DTO at the edge (`fromJson`), returning a DTO and holding no logic beyond that; it never returns a raw `Map`. The repository implementation owns DTO→entity mapping and the single error-to-`Failure` boundary, and a parse fault thrown at the edge propagates into that boundary's `try/catch`. This seam stays an internal `data`-layer detail: it is never exported, never referenced from `domain`, `presentation`, or `core`, and does not become a second layer-crossing abstraction — the repository interface remains the only one. It may be collapsed back into the repository implementation once a mocking library makes the SDK directly fakeable.
...
Client-side validation mirrors the field constraints documented in the briefs — for example the `display_name` 1–50 and `bio` 150-character limits in the profile brief, or the `INVALID_REQUEST` error in the connections brief — for fast UX feedback before a round trip. The backend constraint is authoritative. A client constraint may also be deliberately *stricter* than the wire contract when it protects the app's own layout rather than mirrors a backend rule — e.g. the bio line-break cap, which keeps a profile's cards on screen. Such UX constraints are product decisions, not contract mirrors: the backend remains free to accept values the client UI declines, and the client must still render any value the backend returns. When the backend surfaces a constraint violation (a Shape-2 SDK error or a Shape-1 `INVALID_REQUEST` code), the data layer maps it to a `Failure` and the presentation layer shows the user a meaningful, localized message.
