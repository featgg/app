...
- **Constraints (surface as the SDK error on violation).**
  ...
  - `settings` is a JSON object up to ~50 KB. Versioned envelope:
    ...
    `"framing": { "x": <0..1>, "y": <0..1>, "scale"?: <1..3> }` — the point in
    the picture the frame keeps when it crops, and how much larger than that
    frame the picture is drawn. Unlike the sub-objects below it is not tied to
    ...
    version. `scale` is itself optional within the field: absent means the size
    at which the picture exactly covers the frame, it is never below that size,
    and — like the field around it — it is additive and never bumps the
