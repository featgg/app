...
**Framing.** A bleed card fills its frame with the art and crops the rest, so on any picture wider or taller than the frame something is lost — and the default middle crop is regularly the wrong thing to keep. The owner moves the art inside its frame, in edit mode, on the card itself, through an **explicit framing mode**: tap the mark on the card to enter it, drag freely with one finger — both axes — and tap the mark again, or anywhere outside the card, to leave. One card frames at a time.

The mode is what lets the card sit in a scrolling page. A freely draggable picture inside a scroll view is a gesture contest the page rightly wins, so outside the mode the picture is inert and the page scrolls exactly as it always did; inside it the card owns the gesture outright. Ownership, not delay — an earlier hold-to-drag answered the same conflict and was retired for it: the hold is the convention for picking a card *up*, not for moving a picture inside one, and it read as arbitrary (the decision of record is on the framing-interaction spike). Pointer input gets the same mode for the same reason: there is no long-press convention on a pointer, and every desktop reference uses a visible mode plus a confirm.
