...
final class ProfileWidgetsRepositoryImpl implements ProfileWidgetsRepository {
  ...
  Future<Either<Failure, ProfileWidget>> addPersonalBestWidget({
    ...
    try {
      ...
      final dto = await _source.insertWidget({
        'platform': wireValue,
        'type': profileWidgetKindToWire(ProfileWidgetKind.personalBest),
        'position': await _freePosition(userId, position),
        'is_enabled': true,
        // Bare envelope: the card renders the peak the platform published for
        // the primary mode, so it carries no per-widget selection sub-object.
        'settings': {'schema_version': kProfileWidgetSettingsVersion},
      });
      final widget = profileWidgetFromDto(dto);
      if (widget == null) {
  ...
  Future<Either<Failure, ProfileWidget>> addRarestAchievementWidget({
    ...
    try {
      ...
      final dto = await _source.insertWidget({
        'platform': wireValue,
        'type': profileWidgetKindToWire(ProfileWidgetKind.rarestAchievement),
        'position': await _freePosition(userId, position),
        'is_enabled': true,
        // Bare envelope: the card renders whatever the platform published as
        // rarest, so it carries no per-widget selection sub-object.
        'settings': {'schema_version': kProfileWidgetSettingsVersion},
      });
      final widget = profileWidgetFromDto(dto);
      if (widget == null) {
